...
def test(dataloader, model, loss_fn, device="cpu"):
    ...
    * (X, y, bias_label, group) 
    ...
    # TODO change so that the dataloader can have only two values (but then you might have to just make a new function)
    with torch.no_grad():
...
def adjacency_test(dataloader, model, loss_fn, device="cpu"):
    """
    Builds an adjanecy matrix between the model predictions and the provided bias label.

    Expects batches of:
    * (X, y, bias_label) or (X, y, bias_label,...)

    If only (X, y) are provided we fall back to using y as the bias/group label, this gives us 
    the adjacency between the prediciton and the true class.
    """
    ...
    test_loss /= num_batches
    correct /= size
    print(f"Adjacency Test: \n Accuracy: {(100*correct):>0.1f}%, Avg loss: {test_loss:>8f} \n")
    # run.log({"acc" : correct, "loss":test_loss})
    return adjacency_matrix


class CMNISTNeuralNetwork(nn.Module): # Move to own file down the line, for now leaving here
    def __init__(self):
        super().__init__()
